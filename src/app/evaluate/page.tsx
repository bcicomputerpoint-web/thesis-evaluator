...
// â”€â”€ Compliance groups â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const GROUPS = [
  {
    id: "ugc_core", label: "UGC Core Regulations", icon: "âš–ï¸", color: C.blue, weight: 20,
    fields: [
      { id: "min_duration", label: "PhD registration date (for duration check)", type: "date_check", benchmark: "â‰¥ 3 years from registration", source: "UGC 2022, Cl.5", pass: (v: string) => !!v && (Date.now() - new Date(v).getTime()) / (1000*60*60*24*365) >= 3 },
      { id: "max_duration_ok", label: "Within maximum duration (â‰¤ 6 years / 8 for women/PwD)", type: "boolean", benchmark: "â‰¤ 6 years", source: "UGC 2022, Cl.5", pass: (v: string) => v === "yes" },
      ...
      { id: "coursework_credits", label: "Coursework credits completed", type: "number", unit: "credits", benchmark: "â‰¥ 12 credits", source: "UGC 2022, Cl.9", pass: (v: string) => Number(v) >= 12 },
      { id: "coursework_marks", label: "Coursework aggregate marks", type: "number", unit: "%", benchmark: "â‰¥ 55%", source: "UGC 2022, Cl.9", pass: (v: string) => Number(v) >= 55 },
      ...
      { id: "teaching_assist", label: "Teaching assistantship (4â€“6 hrs/week)", type: "select", options: ["Yes â€” completed","Exempted (valid reason)","Not completed"], benchmark: "4â€“6 hrs/week mandatory", source: "UGC 2022, Cl.9(2)", pass: (v: string) => v && v !== "Not completed" },
  ...
  {
    id: "rac_drc", label: "RAC / DRC Compliance", icon: "ðŸ›ï¸", color: C.red, weight: 15,
    fields: [
      ...
      { id: "rac_constituted", label: "RAC formally constituted (â‰¥ 3 members)", type: "boolean", benchmark: "Mandatory â€” supervisor as convener", source: "UGC 2022, Cl.10", pass: (v: string) => v === "yes" },
      { id: "rac_reports", label: "6-monthly RAC progress reports submitted", type: "number", unit: "reports", benchmark: "â‰¥ 1 per 6 months", source: "UGC 2022, Cl.10", pass: (v: string) => Number(v) >= 1 },
      ...
      { id: "intl_visit", label: "Inter-institutional research visit", type: "select", options: ["Completed (â‰¥ 3 weeks)","Planned (RAC-approved)","Not undertaken"], benchmark: "Recommended â‰¥ 3 weeks", source: "UGC 2022, Cl.9.6", pass: (v: string) => v && v !== "Not undertaken" },
    ]
  },
  {
    id: "supervisor", label: "Supervisor Eligibility", icon: "ðŸ‘©â€ðŸ«", color: C.orange, weight: 10,
    fields: [
      { id: "sup_designation", label: "Supervisor designation", type: "select", options: ["Professor","Associate Professor","Assistant Professor (â‰¥5 yrs)","Retired (re-appointed â‰¤70)","Adjunct/Visiting (limited)"], benchmark: "Prof/AssocProf: â‰¥5 pubs; Asst Prof: â‰¥3 pubs + â‰¥5 yrs exp", source: "UGC 2022, Cl.6.1", pass: (v: string) => !!v },
      { id: "sup_publications", label: "Supervisor peer-reviewed publications (post-PhD)", type: "number", unit: "papers", benchmark: "Prof/Assoc: â‰¥ 5 Â· Asst Prof: â‰¥ 3", source: "UGC 2022, Cl.6.1", pass: (v: string) => Number(v) >= 3 },
      { id: "sup_load", label: "Total scholars under this supervisor", type: "number", unit: "scholars", benchmark: "â‰¤ 8 total (incl. co-supervisions)", source: "UGC 2022, Cl.6.3", pass: (v: string) => Number(v) <= 8 },
  ...
  {
    id: "pub_ethics", label: "Publications & Ethics", icon: "ðŸ“„", color: C.gold, weight: 15,
    fields: [
      { id: "pub_count", label: "Papers published / accepted before submission", type: "number", unit: "papers", benchmark: "â‰¥ 1 in UGC CARE / Scopus / WoS", source: "UGC 2022, Cl.13", pass: (v: string) => Number(v) >= 1 },
      { id: "journal_type", label: "Journal indexing status", type: "select", options: ["UGC CARE Group I","UGC CARE Group II (Scopus/WoS)","Scopus only (verify with CARE)","Not indexed / predatory (HIGH RISK)"], benchmark: "Must be CARE Group I or Group II", source: "UGC CARE â€” ugccare.unipune.ac.in", pass: (v: string) => v && !v.includes("predatory") && !v.includes("HIGH RISK") },
      ...
      { id: "conference_pres", label: "Conference presentations", type: "number", unit: "presentations", benchmark: "â‰¥ 2 recommended (1 national + 1 intl.)", source: "Standard PhD requirements", pass: (v: string) => Number(v) >= 1 },
      { id: "ai_disclosure", label: "AI-generated content disclosure", type: "select", options: ["No AI tools used","AI tools used â€” fully disclosed","AI for coding/data only (disclosed)","AI used â€” not disclosed (HIGH RISK)"], benchmark: "UGC 2025: undisclosed AI = misconduct", source: "UGC AI guidelines 2024-25", pass: (v: string) => v && !v.includes("not disclosed") },
      ...
      { id: "ipr_disclosed", label: "IPR potential disclosed to RAC/university", type: "select", options: ["No IPR potential","IPR potential disclosed and documented","IPR potential exists â€” not disclosed (issue)"], benchmark: "Notify university before viva if patentable", source: "UGC 2022, Cl.9.7", pass: (v: string) => v && v !== "IPR potential exists â€” not disclosed (issue)" },
    ]
  },
  {
    id: "plagiarism", label: "Plagiarism Compliance", icon: "ðŸ”", color: C.orange, weight: 10,
    fields: [
      { id: "similarity", label: "Overall similarity score", type: "number", unit: "%", benchmark: "< 10% (Level 0 â€” safe to submit)", source: "UGC Anti-Plagiarism 2018", pass: (v: string) => Number(v) < 10 },
      { id: "plag_tool", label: "Detection tool used", type: "select", options: ["DrillBit-Extreme (ShodhShuddhi â€” current)","Turnitin","iThenticate","Urkund (legacy)","Other"], benchmark: "DrillBit-Extreme mandatory since Oct 2023", source: "INFLIBNET ShodhShuddhi portal", pass: (v: string) => !!v },
      ...
      { id: "biblio_excluded", label: "Bibliography excluded from similarity calculation", type: "boolean", benchmark: "Standard â€” bibliography excluded from 10% threshold", source: "UGC Anti-Plagiarism 2018 exclusion clause", pass: (v: string) => v === "yes" },
  ...
  {
    id: "shodhganga", label: "Shodhganga / INFLIBNET", icon: "ðŸ—„ï¸", color: C.purple, weight: 8,
    fields: [
      ...
      { id: "file_naming", label: "INFLIBNET file naming convention followed", type: "boolean", benchmark: "01_title_ Â· 02_cert_ Â· 03_abstract_â€¦", source: "INFLIBNET Technical Guidelines", pass: (v: string) => v === "yes" },
  ...
  {
    id: "structure", label: "Thesis Structure", icon: "ðŸ“š", color: C.green, weight: 7,
    fields: [
      { id: "prelim_pages", label: "All mandatory preliminary pages present", type: "boolean", benchmark: "Cover Â· Title Â· Declaration Â· Supervisor Certificate Â· ToC Â· Abstract", source: "IIT Bombay / IIT Madras guidelines", pass: (v: string) => v === "yes" },
      { id: "chapter_order", label: "Standard chapter structure followed", type: "boolean", benchmark: "Intro â†’ LR â†’ Methodology â†’ Results â†’ Discussion â†’ Conclusion", source: "Standard doctoral thesis structure", pass: (v: string) => v === "yes" },
      { id: "chapter_count", label: "Number of chapters", type: "number", unit: "chapters", benchmark: "5â€“7 chapters (excluding prelim and end matter)", source: "Standard doctoral structure", pass: (v: string) => Number(v) >= 4 && Number(v) <= 10 },
      { id: "citation_style", label: "Citation style consistently applied", type: "select", options: ["APA 7th","Harvard","Chicago 17th","MLA 9th","Vancouver","IEEE","Discipline-specific (documented)","Mixed â€” non-compliant"], benchmark: "Single format throughout â€” no mixing", source: "Examiner rubrics across Indian universities", pass: (v: string) => v && v !== "Mixed â€” non-compliant" },
  ...
  {
    id: "examiner_process", label: "Examiner & Viva Process", icon: "ðŸŽ“", color: "#14B8A6", weight: 8,
    fields: [
      { id: "ext_examiners", label: "External examiners appointed", type: "number", unit: "examiners", benchmark: "â‰¥ 2 (at least 1 from outside state/country)", source: "UGC 2022, Cl.14", pass: (v: string) => Number(v) >= 2 },
      ...
      { id: "rejection_protocol", label: "Examiner rejection protocol status", type: "select", options: ["Both accepted (proceed to viva)","One rejected â€” alternate sent","Both/alternate rejected â€” thesis at risk","Not at this stage yet"], benchmark: "1 rejection â†’ alternate. Both reject â†’ thesis rejected.", source: "UGC 2022, Cl.14", pass: (v: string) => v && !v.includes("at risk") },
      { id: "viva_open", label: "Open viva-voce scheduled / completed", type: "boolean", benchmark: "Must be open â€” notice circulated publicly", source: "UGC 2022, Cl.14; IIT Indore", pass: (v: string) => v === "yes" },
    ]
  },
  {
    id: "inclusivity", label: "Inclusivity & Welfare", icon: "ðŸŒ¸", color: "#EC4899", weight: 7,
    fields: [
      { id: "scholar_category", label: "Scholar category", type: "select", options: ["General (full-time)","SC/ST/OBC scholar","Woman scholar","Person with Disability (PwD >40%)","Part-time (working professional)","International scholar"], benchmark: "Determines applicable extensions and provisions", source: "UGC 2022, Cl.3â€“5", pass: (v: string) => !!v },
      { id: "maternity_leave", label: "Maternity / child care leave documented (women scholars)", type: "select", options: ["Not applicable","Availed â€” documented","Entitled but not availed","Not aware of entitlement (issue)"], benchmark: "Up to 240 days â€” not deducted from research time", source: "UGC 2022, Cl.5(2)", pass: (v: string) => v && v !== "Not aware of entitlement (issue)" },
      { id: "noc_parttime", label: "NOC from employer (part-time scholars)", type: "select", options: ["Full-time (not required)","Part-time â€” NOC submitted","Part-time â€” NOC pending (issue)","Part-time â€” self-employed"], benchmark: "Mandatory for part-time scholars with employer", source: "UGC 2022, Cl.13", pass: (v: string) => v && v !== "Part-time â€” NOC pending (issue)" },
...
function getVerdict(s: number) {
  if (s >= 85) return { label: "Category A â€” Ready", short: "Cat. A", color: C.green, cat: "A" as EvalCategory };
  if (s >= 65) return { label: "Category B â€” Minor Revisions", short: "Cat. B", color: C.gold, cat: "B" as EvalCategory };
  if (s >= 45) return { label: "Category C â€” Major Revisions", short: "Cat. C", color: C.orange, cat: "C" as EvalCategory };
  return { label: "Category F â€” Not Ready", short: "Cat. F", color: C.red, cat: "F" as EvalCategory };
}

// â”€â”€ Field input component â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function FieldInput({ field, value, onChange }: { field: any, value: any, onChange: (v: string) => void }) {
  ...
  if (field.type === "select") {
    return <select value={value || ""} onChange={e => onChange(e.target.value)} style={{ ...base, width: "100%", cursor: "pointer" }}><option value="">Selectâ€¦</option>{field.options.map((o: string) => <option key={o}>{o}</option>)}</select>;
  }
  if (field.type === "date_check") {
    ...
    return (
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        {yrs && <span style={{ fontSize: 11, color: ok ? C.green : C.red, fontWeight: 700 }}>{ok ? `âœ“ ${yrs} yrs â€” meets minimum` : `âœ— ${yrs} yrs â€” below 3-year minimum`}</span>}
      </div>
    );
  }
  return (
...
export default function EvaluatePage() {
  ...
  if (loading || !user) return <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center" }}><div style={{ color: C.sec }}>Loadingâ€¦</div></div>;
  ...
  const Header = () => (
    <header style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, padding: "10px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <Link href="/dashboard" style={{ color: C.sec, textDecoration: "none", fontSize: 13 }}>â† Dashboard</Link>
        ...
        <span style={{ fontSize: 11, color: C.sec }}>
          {["Engine","Details","Compliance","Upload","Report"].map((s, i) => (
            <span key={s}>
              ...
              {i < 4 && <span style={{ color: C.muted }}> â†’ </span>}
            </span>
          ))}
        </span>
        {step >= 2 && (
  ...
  // â”€â”€ STEP 0: ENGINE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  if (step === 0) return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "Inter, sans-serif", color: C.text }}>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "32px 18px" }}>
        ...
        <p style={{ color: C.sec, fontSize: 13, marginBottom: 24 }}>All engines evaluate against the same 9 groups Â· 52 criteria. API keys are stored server-side only.</p>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 24 }}>
          {[
            { id: "claude" as AIEngine, name: "Claude", color: C.claude, icon: "ðŸŸ ", desc: "Artifact native Â· Zero extra key needed Â· Best structured JSON Â· 200K context" },
            { id: "gemini" as AIEngine, name: "Gemini", color: C.gemini, icon: "ðŸ”µ", desc: "1M token context Â· Entire thesis in one call Â· Free tier Â· Native PDF" },
            { id: "hybrid" as AIEngine, name: "Hybrid", color: C.hybrid, icon: "âš¡", desc: "Gemini reads full PDF (1M) Â· Claude writes structured report Â· Best quality" },
          ].map(e => (
            <div key={e.id} onClick={() => setEngine(e.id)} style={{ ...S.card, cursor: "pointer", borderColor: engine === e.id ? e.color : C.border, borderWidth: engine === e.id ? 2 : 1, background: engine === e.id ? e.color + "0D" : C.card }}>
              ...
              {engine === e.id && <div style={{ marginTop: 8, fontSize: 10, fontWeight: 700, color: e.color }}>SELECTED âœ“</div>}
            </div>
          ))}
        </div>
        <button style={S.btn(C.blue)} onClick={() => setStep(1)}>Continue with {engine.charAt(0).toUpperCase() + engine.slice(1)} â†’</button>
      </div>
    </div>
  );

  // â”€â”€ STEP 1: THESIS DETAILS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  if (step === 1) return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "Inter, sans-serif", color: C.text }}>
      <Header />
      <div style={{ maxWidth: 660, margin: "0 auto", padding: "28px 18px" }}>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "space-between", marginTop: 22 }}>
          <button style={S.ghost} onClick={() => setStep(0)}>â† Back</button>
          <button style={S.btn(C.blue)} onClick={() => setStep(2)} disabled={!meta.title || !meta.scholar}>Next: Compliance â†’</button>
        </div>
      </div>
    </div>
  );

  // â”€â”€ STEP 2: COMPLIANCE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  if (step === 2) {
        ...
        <div style={{ maxWidth: 940, margin: "0 auto", padding: "16px", display: "grid", gridTemplateColumns: "190px 1fr", gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ ...S.card, marginTop: 10, textAlign: "center", padding: 12 }}>
              ...
              <div style={{ fontSize: 10, color: C.muted, marginBottom: 5 }}>/ 100 Â· 52 criteria</div>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 13 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: C.sec }}>Weight: {GW[ag.id]}% Â· {ag.fields.length} criteria</div>
              </div>
            </div>
                  ...
                  <div key={field.id} style={{ ...S.card, borderLeft: `3px solid ${has ? (passed ? C.green : C.red) : C.border}`, padding: 14 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 7, gap: 8 }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 11, color: C.sec }}>Benchmark: <strong style={{ color: C.text }}>{field.benchmark}</strong> Â· <span style={{ color: C.blue }}>{field.source}</span></div>
                      </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 12, gap: 10 }}>
              <button style={S.ghost} onClick={() => { if (ai > 0) setActiveGroup(GROUPS[ai - 1].id); else setStep(1); }}>{ai > 0 ? "â† Prev Section" : "â† Back"}</button>
              <button style={S.btn(C.blue)} onClick={() => { if (ai < GROUPS.length - 1) setActiveGroup(GROUPS[ai + 1].id); else setStep(3); }}>
                {ai < GROUPS.length - 1 ? `Next: ${GROUPS[ai + 1].label} â†’` : "Upload PDF â†’"}
              </button>
            </div>
          </div>
        </div>
  ...
  // â”€â”€ STEP 3: UPLOAD + ANALYSE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  if (step === 3) return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "Inter, sans-serif", color: C.text }}>
      <Header />
      <div style={{ maxWidth: 580, margin: "0 auto", padding: "32px 18px" }}>
        ...
        <div onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f) handleFile(f); }} onClick={() => fileRef.current?.click()} style={{ border: `2px dashed ${pdfFile ? C.green : C.border}`, borderRadius: 12, padding: "32px 18px", textAlign: "center", cursor: "pointer", background: pdfFile ? C.green + "08" : C.surface, marginBottom: 20 }}>
          ...
          <div style={{ fontSize: 28, marginBottom: 8 }}>{pdfFile ? "âœ…" : "ðŸ“„"}</div>
          {pdfFile ? <><div style={{ fontWeight: 700, color: C.green }}>{pdfFile.name}</div><div style={{ color: C.sec, fontSize: 12, marginTop: 4 }}>Click to replace</div></> : <><div style={{ fontWeight: 600 }}>Drop PDF here or click to browse</div><div style={{ color: C.sec, fontSize: 12, marginTop: 4 }}>Optional â€” enables AI structural analysis. Max 50MB.</div></>}
        </div>
        ...
        <button onClick={runAnalysis} disabled={aiLoading} style={{ ...S.btn(C.blue), width: "100%", padding: 13, fontSize: 14, opacity: aiLoading ? 0.6 : 1 }}>
          {aiLoading ? "â³ Running AI Analysisâ€¦" : "Run AI Evaluation â†’"}
        </button>
        <button style={{ ...S.ghost, marginTop: 10, width: "100%" }} onClick={() => setStep(2)}>â† Back to Compliance</button>
      </div>
    </div>
  );

  // â”€â”€ STEP 4: REPORT â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  if (step === 4 && aiResult) {
      ...
      <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "Inter, sans-serif", color: C.text }}>
        ...
        <div style={{ maxWidth: 880, margin: "0 auto", padding: "16px" }}>
          ...
          <div style={{ ...S.card, borderTop: `3px solid ${vrd.color}`, marginBottom: 12, padding: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ color: C.sec, fontSize: 12, marginTop: 5 }}>{meta.scholar} Â· {meta.university} Â· {meta.year}</div>
              </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
            ...
            <div style={S.card}>
              <div style={{ fontSize: 10, fontWeight: 700, color: C.green, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 10 }}>âœ… Strengths</div>
              {(aiResult.strengths || []).map((s: string, i: number) => <div key={i} style={{ display: "flex", gap: 7, marginBottom: 6 }}><span style={{ color: C.green }}>â–¸</span><span style={{ fontSize: 13, lineHeight: 1.6 }}>{s}</span></div>)}
            </div>
            <div style={{ ...S.card, borderTop: `3px solid ${catC[aiResult.examiner_readiness?.category] || C.border}` }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 7 }}>ðŸŽ“ Examiner Readiness</div>
              ...
              {(aiResult.examiner_readiness?.viva_risk_areas || []).map((a: string, i: number) => <div key={i} style={{ fontSize: 12, color: C.sec, marginBottom: 2 }}>âš  {a}</div>)}
            </div>
          </div>

          {(aiResult.critical_gaps || []).length > 0 && (
            <div style={{ ...S.card, marginBottom: 12 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: C.red, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 10 }}>ðŸ”§ Critical Gaps ({aiResult.critical_gaps.length})</div>
                ...
                <div key={i} style={{ background: C.bg, borderRadius: 8, padding: "11px 13px", borderLeft: `3px solid ${priC[gap.priority] || C.border}`, marginBottom: 8 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 12, color: C.blue }}>â†’ {gap.action}</div>
          ...
          <div style={{ ...S.card, marginBottom: 12 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 12 }}>ðŸ“Š Compliance Breakdown â€” 9 Groups</div>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
            <Link href="/dashboard" style={{ background: C.card, border: `1px solid ${C.border}`, color: C.sec, textDecoration: "none", fontSize: 13, padding: "10px 18px", borderRadius: 8 }}>â† Dashboard</Link>
            <button onClick={() => window.print()} style={S.btn(C.blue)}>ðŸ–¨ Print / Save PDF</button>
          </div>

          <div style={{ textAlign: "center", fontSize: 10, color: C.muted, marginTop: 20, lineHeight: 2 }}>
            Onusandhan Â· Evaluated: {new Date().toLocaleDateString("en-IN", { day: "2-digit", month: "long", year: "numeric" })} Â· UGC 2022 Â· Anti-Plagiarism 2018 Â· Shodhganga Jan 2024 Â· Category A/B/C/F Framework
          </div>
        </div>
      </div>
    );
